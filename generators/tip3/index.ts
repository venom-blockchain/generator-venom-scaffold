  ...
  async prompting() {
    ...
    if (this.options.locklift) {
      ...
      const contractAnswers = {
        rootOwnerAddress:
          answers.rootOwnerAddress || this.options.rootOwnerAddress,
        ...
        initialSupplyTo:
          answers.initialSupplyTo || this.options.initialSupplyTo,
        ...
        disableMint:
          this.options.disableMint === undefined
            ? answers.disableMint
            : this.options.disableMint,
        disableBurn:
          this.options.disableBurn === undefined
            ? answers.disableBurn
            : this.options.disableBurn,
        pauseBurn:
          this.options.pauseBurn === undefined
            ? answers.pauseBurn
            : this.options.pauseBurn,
      };
    ...
    if (this.options.locklift) {
      const pkgJsonScripts = {
        scripts: {
          ...
          'deploy:testnet':
            'npx locklift run --network testnet --script scripts/00-deploy.ts',
          'deploy:mainnet':
            'npx locklift run --network mainnet --script scripts/00-deploy.ts',
          'mint:testnet':
            'npx locklift run --network testnet --script scripts/01-mint.ts',
          'mint:mainnet':
            'npx locklift run --network mainnet --script scripts/01-mint.ts',
          'burn:testnet':
            'npx locklift run --network testnet --script scripts/02-burn.ts',
          'burn:mainnet':
            'npx locklift run --network mainnet --script scripts/02-burn.ts',
        },
        devDependencies: {
          ora: '^4.0.0',
          prompts: '^2.4.2',
