...
describe('Test Awesome token deployment, minting and burning', async function () {
  ...
  it('Load TokenRoot contract factory', async function () {
    const tokenRootSampleData =
      locklift.factory.getContractArtifacts('TokenRoot');
    expect(tokenRootSampleData.code).not.to.equal(
      undefined,
      'Code should be available',
    );
    expect(tokenRootSampleData.abi).not.to.equal(
      undefined,
      'ABI should be available',
    );
    expect(tokenRootSampleData.tvc).not.to.equal(
      undefined,
      'tvc should be available',
    );
  });
  it('Load TokenWallet contract factory', async function () {
    const tokenWalletSampleData =
      locklift.factory.getContractArtifacts('TokenWallet');
    expect(tokenWalletSampleData.code).not.to.equal(
      undefined,
      'Code should be available',
    );
    expect(tokenWalletSampleData.abi).not.to.equal(
      undefined,
      'ABI should be available',
    );
    expect(tokenWalletSampleData.tvc).not.to.equal(
      undefined,
      'tvc should be available',
    );
  });
  it(`Deploy TokenRoot contract`, async function () {
    ...
    tokenRoot = contract;
    const { value0: balance } = await tokenRoot.methods
