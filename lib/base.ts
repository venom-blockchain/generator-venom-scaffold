...
export class PkgJSONGenerator extends Generator {
  ...
  async _promptPkgJSON() {
    ...
    const questions = this.pkgJSON
      ? this._notAnsweredQuestions()
      : this._allQuestions();
    ...
    this.pkgManager = this.answers.pkgManager
      ? this.answers.pkgManager
  ...
  _writePkgJSON() {
    const name =
      this.pkgJSON?.name || this.answers?.pkgName || this.options?.pkgName;
    const pkgJSON = {
      name:
        this.pkgJSON?.name || this.answers?.pkgName || this.options?.pkgName,
      description:
        this.pkgJSON?.description ||
        this.answers?.description ||
        this.options?.description,
      license:
        this.pkgJSON?.license || this.answers?.license || this.options?.license,
      author:
        this.pkgJSON?.author || this.answers?.author || this.options?.author,
      homepage:
        this.pkgJSON?.homepage ||
        this.answers?.homepage ||
        this.options?.homepage,
      keywords:
        this.pkgJSON?.keywords ||
...
export default class BaseGenerator extends Generator {
  ...
  _readConfigFile(path: string) {
    const configPath = p.isAbsolute(path)
      ? path
      : p.join(this.contextRoot, path);
